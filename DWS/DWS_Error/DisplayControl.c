...
void DisplaySetting(TIME* time, MODE* mode) //TImeSetting함수
{
	//////////////////////////////
	char s1[3];
	char s2[3];
...
void LapTimeReset(STOPWATCH* sw, MODE* md)	//랩타임 리셋 //------------------Detect
{
	sw->Lap_min = 0;
	sw->Lap_se = 0;
	sw->Lap_Cse = 0;
...
void IncreaseAlarmValue(ALARM* r, MODE* m) {
	...
	else if (m->AL_minute == 1) { //59,  60x 0~59
		r->minute++;//60
		if (r->minute >= 60) {//들어감
			r->minute = 0;//초기화
		}
	}
	else if (m->AL_month == 1) { //11, 12,13x 0~12
		r->month++;//13
		if (r->month > 12) {//들어감
			r->month = 1;//초기화                      
		}
	...
	else if (m->AL_day == 1)
	{
		...
		{
			if (r->day > 31) {
				r->day = 1;    //31일인 달 31일이 넘으면 달 하나 다음걸로
			}
		}
		else if (r->month == 4 || r->month == 6 || r->month == 9 || r->month == 11) {
			if (r->day > 30) {
				r->day = 1;   //30일인달 넘으면 하나 다음걸로
			}
		}
